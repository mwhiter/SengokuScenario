...
Controls.StartButton:RegisterCallback(Mouse.eLClick, function()
	...
	local iModVersion = Modding.GetActivatedModVersion(MOD_ID);
	local scenarioMap = Modding.GetEvaluatedFilePath(MOD_ID, iModVersion, MAP_FILE);
...
function SetPlayerIndex(playerIndex)
	...
	if(playerIndex ~= nil) then
		...
		-- Sets Dawn of Man Quote
		Controls.Quote:LocalizeAndSetText("TXT_KEY_SENGOKU_SCENARIO_CIV5_DAWN_TEXT");		
    end
